...
public class AddressRecyclerview extends RecyclerView {
    ...
    public AddressRecyclerview(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AddressRecyclerview(Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (status == CLOSE) {
                    ...
                    mItemView = viewHolder.getView(R.id.adapter_address_item);
                    mPosition = viewHolder.getAdapterPosition();
                    mLayoutView = viewHolder.getView(R.id.adapter_address_layout);
                    mDeleteView = viewHolder.getView(R.id.adapter_address_delete);
                    mEditView = viewHolder.getView(R.id.adapter_address_edit);
                    mDefaulView = viewHolder.getView(R.id.adapter_address_defaul);
                    mMaxLength = mLayoutView.getWidth();

                    mDefaulView.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            ...
                            status = CLOSE;
                            if (mListener != null) {
                                mListener.onDefualClick(mPosition);
                            }
                        }
                    });
